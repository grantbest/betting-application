...
def purge_mock_data():
    if APP_ENV != "production":
        print(
            f"Current environment is {APP_ENV}. This script is intended for PRODUCTION cleanup."
        )
    ...
    try:
        ...
        cur.execute(
            "DELETE FROM bet_tracking WHERE game_id = ANY(%s)", (MOCK_GAME_IDS,)
        )
        bets_deleted = cur.rowcount

        conn.commit()
        print(
            f"✅ Purged {logs_deleted} mock inning logs and {bets_deleted} mock bets."
        )
