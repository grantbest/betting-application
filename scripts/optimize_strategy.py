...
def fetch_data():
    ...
    try:
        ...
        cur.execute(
            "SELECT rule_id, name, conditions_json FROM betting_rules WHERE status = 'ACTIVE'"
        )
        rules = [{"id": r[0], "name": r[1], "conditions": r[2]} for r in cur.fetchall()]

        cur.execute(
            "SELECT game_id, system_triggered, result FROM bet_tracking WHERE result != 'PENDING' ORDER BY created_at DESC LIMIT 100"
        )
        history = [
            {"game_id": h[0], "system": h[1], "result": h[2]} for h in cur.fetchall()
        ]
...
if __name__ == "__main__":
    ...
    if not rules or not history:
        print(
            "Not enough data to analyze. Ensure you have active rules and completed bets."
        )
        sys.exit(1)
